...
<template>
  ...
  >
    ...
    >
      ...
      <span v-if="mode === 'edit' && dueCount > 0" class="bg-red-50 dark:bg-red-950 text-red-600 dark:text-red-300 text-[10px] px-1.5 py-0.5 rounded-full font-bold leading-none flex-shrink-0">{{ dueCount }}</span>
    </button>
    ...
    <template v-if="mode === 'edit'">
      <!-- Search -->
      <div class="flex-1 flex items-center gap-2 bg-[#f7f8fa] dark:bg-gray-800 rounded-lg focus-within:bg-white dark:focus-within:bg-gray-700 focus-within:ring-2 focus-within:ring-accent/20 transition-all"
      ...
      <button
        v-if="activeId"
        style="-webkit-app-region: no-drag;"
        class="flex items-center gap-1 rounded-md border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 text-[13px] transition-all duration-200 ease-out active:scale-95"
        :class="[
          canGoPrev ? 'cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 dark:bg-gray-800' : 'opacity-40 pointer-events-none',
          compact ? 'px-2 py-1.5' : 'px-2.5 py-1.5',
        ]"
        title="上一题 (Ctrl+←)"
        @click="emit('prev')"
      ...
      <button
        ...
        class="flex items-center gap-1 rounded-md border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 text-[13px] transition-all duration-200 ease-out active:scale-95"
        :class="[
          canGoNext ? 'cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 dark:bg-gray-800' : 'opacity-40 pointer-events-none',
          compact ? 'px-2 py-1.5' : 'px-2.5 py-1.5',
        ]"
        title="下一题 (Ctrl+→)"
        @click="emit('next')"
      ...
      >
        ...
        <span
          ...
          :class="compact ? 'max-w-0 opacity-0' : 'max-w-24 opacity-100'"
        >保存</span>
        <span v-if="isDirty" class="w-1.5 h-1.5 rounded-full bg-white dark:bg-gray-900 ml-0.5 flex-shrink-0" />
      </button>
      ...
      <div class="relative flex-shrink-0">
        <button
          style="-webkit-app-region: no-drag;"
          class="flex items-center justify-center w-9 h-9 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-500 dark:text-gray-300 hover:text-gray-700 dark:hover:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-700 transition-all"
          title="更多"
