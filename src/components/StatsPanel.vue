...
<template>
  ...
  <!-- Panel -->
  <div class="fixed right-0 top-0 bottom-0 z-50 w-[340px] bg-white shadow-xl border-l border-gray-200 overflow-y-auto">
    <!-- Header -->
    ...
    <div class="p-5 space-y-6">
      ...
      <div>
        <h3 class="text-[12px] font-semibold text-gray-500 mb-3 uppercase tracking-[0.5px]">学科分布</h3>
        <div v-if="stats.subjectBars.value.length === 0" class="text-[12px] text-gray-400 text-center py-4">
          暂无数据
        </div>
        <div v-else class="space-y-2">
          ...
          >
            <span class="text-[12px] text-gray-600 w-[60px] text-right truncate flex-shrink-0" :title="item.name">
              {{ item.name }}
            </span>
            ...
            <span class="text-[11px] text-gray-400 tabular-nums w-5 text-right flex-shrink-0">{{ item.count }}</span>
          </div>
        </div>
      </div>
      ...
      <!-- Weekly activity -->
      <div>
        <h3 class="text-[12px] font-semibold text-gray-500 mb-3 uppercase tracking-[0.5px]">近7天复习</h3>
      ...
      <div>
        <h3 class="text-[12px] font-semibold text-gray-500 mb-3 uppercase tracking-[0.5px]">掌握程度</h3>
        <div v-if="stats.easeBuckets.value.every(b => b.count === 0)" class="text-[12px] text-gray-400 text-center py-4">
          暂无复习数据
        </div>
        <div v-else class="space-y-2">
          <div
            ...
            class="flex items-center gap-2"
          >
            <span class="text-[12px] text-gray-600 w-[48px] text-right flex-shrink-0">
              {{ bucket.label }}
            </span>
            ...
            <span class="text-[11px] text-gray-400 tabular-nums w-6 text-right flex-shrink-0">{{ bucket.count }}</span>
          </div>
        </div>
      </div>
    </div>
