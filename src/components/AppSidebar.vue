...
<!-- @AI-VIEW: DOM 可自由重构。样式仅限 Tailwind CSS 工具类。严禁内联 style 或自定义 CSS。 -->
  ...
  >
    ...
    <div class="flex items-center px-4 py-3 border-b border-gray-100 dark:border-gray-800" :class="collapsed ? 'justify-center' : 'justify-between'">
      <!-- Logo (hidden when collapsed) -->
      <div v-if="!collapsed" class="text-xl font-bold text-accent flex items-center gap-2 overflow-hidden whitespace-nowrap">
        <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/>
        </svg>
        <span class="transition-opacity duration-300" :class="collapsed ? 'opacity-0' : 'opacity-100'">错题本</span>
      </div>

      <button
        class="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-lg text-gray-500 dark:text-gray-300 hover:text-gray-700 dark:hover:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-800 border border-gray-200 dark:border-gray-600 transition-all duration-200"
    ...
    <div v-if="collapsed" class="flex-1 flex flex-col items-center gap-5 pt-5">
      ...
      <!-- Review button -->
      <button
        class="w-9 h-9 flex items-center justify-center rounded-lg text-gray-500 dark:text-gray-300 hover:text-accent hover:bg-accent/10 border border-gray-200 dark:border-gray-600 transition-all duration-200 relative"
        title="开始复习"
      ...
      <!-- Entry count -->
      <span class="text-[11px] text-gray-500 dark:text-gray-300 font-medium tabular-nums" title="题目总数">
      ...
      <!-- Settings -->
      <button
        class="w-9 h-9 flex items-center justify-center rounded-full text-gray-400 dark:text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 border border-gray-200 dark:border-gray-600 transition-all duration-200 mt-auto mb-4"
        title="设置"
    ...
    <template v-else>
      <div class="p-4 border-b border-gray-100 dark:border-gray-800">
        ...
        <div v-if="notebookName" class="flex items-center gap-2 mb-3">
          ...
          <span class="text-[13px] font-medium text-gray-700 dark:text-gray-200 truncate">{{ notebookName }}</span>
        </div>
        ...
        <!-- Mastery filter -->
        <div class="mt-3">
          <h3 class="text-[12px] text-gray-500 dark:text-gray-400 font-semibold uppercase tracking-[0.5px] mb-2">掌握程度</h3>
          <div class="flex flex-wrap gap-1">
            <button
              v-for="b in MASTERY_LEVEL_DEFS"
              :key="b.label"
              class="relative overflow-hidden px-2.5 py-1 rounded-full text-[12px] font-medium transition-all duration-300 ease-out active:scale-95 border-l-[3px]"
        ...
        <div class="px-4 py-2.5 text-[11px] text-gray-400 dark:text-gray-500 font-semibold uppercase tracking-[0.5px] flex justify-between items-center">
          ...
          <div class="relative">
            ...
            >
              ...
              <span v-if="sortKey !== 'shuffle'" class="text-[10px]">{{ sortDir === 'asc' ? '↑' : '↓' }}</span>
            </button>
            ...
            >
              ...
              >
                <span class="flex items-center gap-1.5">
                  <svg v-if="opt.key === 'shuffle'" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="15" y1="15" x2="21" y2="21"/><line x1="4" y1="4" x2="9" y2="9"/></svg>
                  {{ opt.label }}
                </span>
                <span
                  v-if="sortKey === opt.key && opt.key !== 'shuffle'"
                  class="text-[10px]"
                >{{ sortDir === 'asc' ? '↑' : '↓' }}</span>
              </button>
            </div>
      ...
      <div class="px-4 py-3 border-t border-gray-100 dark:border-gray-800">
        <button
          class="w-full flex items-center gap-2.5 px-2.5 py-1.5 rounded-[8px] text-[13px] text-[#555] dark:text-[#bbb] hover:text-[#3a3a3a] dark:text-[#ccc] hover:bg-[#f5f4f2] dark:hover:bg-[#28282c] transition-colors"
          @click="emit('toggle-settings')"
        >
